...
$(document).ready(function(){
  var producers = getDirectories( '../app.asar.unpacked/app/devices/');
...
function updateList(producer){
  var products = getDirectories(  '../app.asar.unpacked/app/devices/'+producer);
...
function appendProduct(producer,name){
  var product=jsyaml.safeLoad(fs.readFileSync(  '../app.asar.unpacked/app/devices/'+producer+'/'+name+'/config.yaml')).product;
  ...
  $('[data-product='+name+'][data-producer='+producer+']').append($('<img/>').addClass('panel-heading').attr({
    src :  '../app.asar.unpacked/app/devices/'+producer+'/'+name+'/'+product.image,
    alt : product.name
  }));
  ...
  $('[data-product='+name+'][data-producer='+producer+']'+" .panel-body").append($('<a/>').addClass('btn btn-primary select-device').attr({
    href : '#',
    'data-device' :   '../app.asar.unpacked/app/devices/'+producer+'/'+name+'/'
  }).text('Next'));
