...
$(document).ready(function(){
  var producers = getDirectories( (__dirname+'/devices/').replace('app.asar','app.asar.unpacked').replace('/selectdevice',''));
...
function appendProduct(producer,name){
  var product=jsyaml.safeLoad(fs.readFileSync( ( __dirname+'/devices/'+producer+'/'+name+'/config.yaml').replace('app.asar','app.asar.unpacked').replace('/selectdevice',''))).product;
  ...
  $('[data-product='+name+'][data-producer='+producer+']').append($('<img/>').addClass('panel-heading').attr({
    src :  (__dirname+'/devices/'+producer+'/'+name+'/'+product.image).replace('app.asar','app.asar.unpacked').replace('/selectdevice',''),
    alt : product.name
  }));
  ...
  $('[data-product='+name+'][data-producer='+producer+']'+" .panel-body").append($('<a/>').addClass('btn btn-primary select-device').attr({
    href : '#',
    'data-device' :   (__dirname+'/devices/'+producer+'/'+name+'/').replace('app.asar','app.asar.unpacked').replace('/selectdevice','')
  }).text('Next'));
